        ...
        <div class="col-lg-8">
          <div class="row">
            ...
            <div class="col-12">
              ...
              <div class="card-body">
                <form action="{{ route('admin.updateCamera', $camera->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <div class="mb-3 mt-3">
                        <label for="nama_barang" class="form-label">Nama Barang</label>
                        <input type="text" name="nama_barang" id="nama_barang" class="form-control" value="{{ $camera->nama_barang }}" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        @if ($camera->gambar)
                            <img src="{{ asset('storage/' . $camera->gambar) }}" alt="Current Image" class="mt-2 img-fluid rounded" style="max-width: 200px;">
                        @endif
                    </div>

                    <div class="mb-3">
                        <label for="harga" class="form-label">Harga</label>
                        <input type="number" name="harga" id="harga" class="form-control" value="{{ $camera->harga }}" required min="0" step="any">
                    </div>

                    <div class="mb-3">
                        <label for="deskripsi" class="form-label">Deskripsi</label>
                        <textarea name="deskripsi" id="deskripsi" class="form-control" rows="5">{{ $camera->deskripsi }}</textarea>
                    </div>
                    <div class="mb-3">
                        <label for="jumlah" class="form-label">jumlah</label>
                        <textarea name="jumlah" id="jumlah" class="form-control" rows="5">{{ $camera->jumlah }}</textarea>
                    </div>
